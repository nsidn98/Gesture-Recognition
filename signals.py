...
'''
library for getting the values with all parameters
'''

class Sample:
    def __init__(self,flex1,flex2,flex3,flex4,flex5,flex6,flex7,angx,angy,angz,acx,acy,acz,gx,gy,gz):
    ...
    def get_linearized(self,reshape=False):
        if reshape:
            return np.concatenate((self.flex1,self.flex2,self.flex3,self.flex4,self.flex5,self.flex6,self.flex7,self.angx,self.angy,self.angz,self.acx, self.acy, self.acz, self.gx, self.gy, self.gz)).reshape(1,-1)
        else:
            return np.concatenate((self.flex1,self.flex2,self.flex3,self.flex4,self.flex5,self.flex6,self.flex7,self.angx,self.angy,self.angz,self.acx, self.acy, self.acz, self.gx, self.gy, self.gz))
    ...
    def load_from_file(filename,size_fit=50):
        ...
        return Sample(flex1_stretch,flex2_stretch,flex3_stretch,flex4_stretch,flex5_stretch,flex6_stretch,flex7_stretch,acx_stretch, acy_stretch, acz_stretch, gx_stretch, gy_stretch, gz_stretch,angx_stretch,angy_stretch,angz_stretch)
